...
此功能让现有 Codex 桌面版使用 AICLI 已配置的两个本地模型。界面、对话历史、工具执行和官方登录仍由原版 Codex 负责。
...
两个本地模型可以在同一本地任务中切换。官方模型在每次桌面引擎启动前由原版引擎重新发现，再与本地元数据合并载入；不维护固定的 OpenAI 模型名单。当前进程使用本次启动快照，新增模型在下一次正常启动时刷新。
...
需要 Windows 11、PowerShell 7、.NET 10 SDK（构建时）及 .NET 10 Runtime（运行时），并已安装 AICLI、Codex 桌面版和两个本地模型。使用 AICLI 当前配置的本地公共网关，沿用已有 GPU 管理。
...
- 本地模型和显示名来自当前 AICLI Profile（配置档案）及模型目录，默认读取 `codex-ollama-main`、`codex-ollama-review`。这些是稳定的内部 ID，界面显示具体型号。
...
- 本地任务通过原生 `thread/start` / `thread/resume` 传入本地连接，并按已载入元数据设置窗口，避免继承大于本地容量的全局上下文设置。本地任务使用同一连接，因此两种 Qwen 可以直接切换。
- 适配器按标准输入输出转发原生 app-server 协议，未知方法、通知、服务端请求和错误继续透传。它不修改桌面安装包或上游 CLI，也不创建后台计划任务或独立聊天界面。

桌面或引擎更新后，应重新验证模型列表、两个本地模型的真实调用和退出清理。源码构建、协议测试、模型调用和前台显示是不同层次的验证，不互相替代。
