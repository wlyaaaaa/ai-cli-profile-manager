...
function Show-AiCliHelp {
    ...
    switch -Regex ($Topic.ToLowerInvariant()) {
        ...
        '^(setup|profile|start|doctor|test|proxy|update|native|eject|uninstall)$' { Show-AiCliHelpCommand -Command $Topic.ToLowerInvariant() }
        default {
            throw "未知帮助主题: $Topic"
        }
    }
...
功能：用 Profile 启动原生 Codex / Claude Code / Open Interpreter，并做代理运维与体检。
...
常用命令：
  ...
  $cmd native <profile>
  $cmd eject <profile>
  $cmd help setup|profile|start|doctor|test|proxy|update|native|eject|uninstall
