...
- Codex Desktop 在读取模型列表时按需刷新官方目录，保留运行中的受管模型、路由和原生元数据；分页绑定同一目录快照，刷新失败继续使用最近一次完整列表。
- 后台 OpenAI 协作新增 Sol High 常规判断入口，与 Astra High 升级判断共用精确五字段合同；普通子任务仍按实际许可独立选择型号、思考等级和上下文复用方式，判断结果不产生执行授权。

### 修复

- 桌面官方模型发现优先使用可用的 npm Codex 刷新原生在线缓存，避免旧桌面引擎遗漏新官方型号。普通账号用量耗尽时保留发送入口，由上游返回实际限额错误；工作区和所有者支出限制不变。
- 加密 AppX 资源场景下解析并验证当前签名桌面引擎及其配套文件，保留完整语义版本比较，不把旧缓存误认作当前引擎。
- 为 11 份含中文的 PowerShell 脚本补齐 UTF-8 BOM，保留原正文和换行；全仓已跟踪脚本的编码合同纳入现有 Pester 检查。
- 顶层 `bin/` 的新增入口不再被 .NET 构建输出规则忽略，嵌套 `bin/obj` 仍排除。

### 维护记录

- 补记双档判断与动态目录联合部署、Luna 首次许可提示修复的分层证据。外部原生 Hook 的修复与本仓库实现分开记录；历史安装和协议测试不代表旧 GUI 已加载新版，也不代表所有真实云端使用已验收，具体边界见维护者归档 25.9、25.10。
...
- 模型沟通策略统一输出 LF，保证 Windows CRLF 检出后的目录生成仍与厂商基线及已提交内容一致；运行控制回执支持普通隐藏祖先目录。
